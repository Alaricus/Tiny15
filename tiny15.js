...
const placePuzzleOnField = () => {
  for (let i = 0; i < 12; i++) {
    ...
    const temp1 = sequence[index1];
    const temp2 = sequence[index2];
    sequence[index1] = temp2;
    sequence[index2] = temp1;
  }
...
canvas.addEventListener('click', () => switchWithZero(p.x, p.y));
canvas.addEventListener('mousemove', e => {
  const crd = getMousep(e);
  p.x = Math.floor(crd.x / 89);
  p.y = Math.floor(crd.y / 89);
});
...
const draw = () => {
  ...
  field.forEach((row, y) => row.forEach((col, x) => {
    if (col.num !== 0) {
      ...
      ctx.fillStyle = x === p.x && y === p.y ? '#ffffff' : '#000000';
      ctx.fillText(
        col.num,
